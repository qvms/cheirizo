...
from os import chdir, chmod, chown, environ, getegid, geteuid, makedirs, walk
...
def on_packages(context: list) -> None:
    ...
    if os_name == "Darwin":
        ...
        packages = " ".join(context)
        privilege = "" if geteuid() == 0 else "sudo "
        run(f"{privilege}apt install -y --no-install-recommends {packages}", check=True, shell=True)
    else:
...
def on_directories(context: list) -> None:
    """Create directories

    Args:
        context (list): List of specs with a "path" and optional octal "permissions".
    """
    for spec in context:
        path = spec["path"]
        try:
            ...
            if spec.get("permissions", None):
                chmod(path, int(spec["permissions"], 8))
        except Exception as e:
            log.error(e)
            exit(-1)


def on_copy_files(context: list) -> None:
    """Copy individual files, preserving metadata

    Args:
        context (list): List of specs with "source", "destination" and optional
            octal "permissions". Copying is idempotent: an existing destination
            is overwritten deterministically.
    """
    for spec in context:
        source = spec["source"]
        ...
        try:
            ...
            chown(destination, geteuid(), getegid())
            if spec.get("permissions", None):
                chmod(destination, int(spec["permissions"], 8))
        except Exception as e:
            log.error(e)
            exit(-1)


def on_copy_trees(context: list) -> None:
    """Copy directory trees

    Args:
        context (list): List of specs with "source" and "destination". Existing
            destination trees are merged/overwritten (dirs_exist_ok) so repeated
            runs are idempotent.
    """
    for spec in context:
        source = spec["source"]
        ...
        try:
            ...
            for root, directories, files in walk(destination):
                chown(root, geteuid(), getegid())
                for name in directories + files:
                    chown(join(root, name), geteuid(), getegid())
        except Exception as e:
            log.error(e)
            exit(-1)
