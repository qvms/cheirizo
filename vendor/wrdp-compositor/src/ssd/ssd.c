...
ssd_create(struct view *view, bool active)
{
	...
	if (os9_platinum_theme_enabled()) {
		/*
		 * Draw the titlebar AFTER (above) the side borders. For squared windows
		 * the left/right borders extend up over the titlebar height; keeping the
		 * titlebar on top stops the side bevel/inset from propagating up into the
		 * titlebar and breaking the top-corner highlights.
		 */
...
ssd_enable_keybind_inhibit_indicator(struct ssd *ssd, bool enable)
{
	...
	if (!part || !part->node || part->node->type != WLR_SCENE_NODE_RECT) {
		return;
	}
