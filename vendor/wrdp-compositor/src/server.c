...
server_global_filter(const struct wl_client *client, const struct wl_global *global, void *data)
{
	...
	if (security_context && global == server->security_context_manager_v1->global) {
		 ...
		 *       protocols wrdp-compositor supports.
		 */
...
/*
 * This message is intended to help users who are trying wrdp-compositor on
 ...
 * exist, in the hope that it will reduce the time required to get wrdp-compositor running
...
"\n"
"If the above does not work, try running with `WLR_RENDERER=pixman wrdp-compositor` in\n"
...
server_init(struct server *server)
{
	 ...
	 * their read fd prematurely to crash wrdp-compositor because of the unhandled
...
server_start(struct server *server)
{
	...
	const char *requested_socket = getenv("WRDP_COMPOSITOR_WAYLAND_DISPLAY");
	const char *socket = NULL;
	if (requested_socket && requested_socket[0]) {
		if (wl_display_add_socket(server->wl_display, requested_socket) == 0) {
			socket = requested_socket;
		} else {
			wlr_log_errno(WLR_ERROR, "unable to open requested wayland socket %s", requested_socket);
			exit(EXIT_FAILURE);
		}
	} else {
