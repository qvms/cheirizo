...
keyboard_get_all_modifiers(struct seat *seat)
{
	 ...
	 * via wayvnc to control wrdp-compositor. This function also gets called to decide when to
		...
		struct wlr_keyboard_key_event *event)
{
	...
	/*
	 * Ignore wrdp-compositor keybindings if the session is locked.
...
keyboard_set_numlock(struct wlr_keyboard *keyboard)
{
	 ...
	 * current wrdp-compositor usage since the keyboard is not part of a
	 * keyboard-group yet).
	 */
