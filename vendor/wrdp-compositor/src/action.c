	...
	wl_list_for_each(action, actions, link) {
		...
		case ACTION_TYPE_KILL:
			if (view) {
				...
				if (pid == getpid()) {
					wlr_log(WLR_ERROR, "Preventing sending SIGTERM to wrdp-compositor");
				} else if (pid > 0) {
					kill(pid, SIGTERM);
				}
			}
		...
		case ACTION_TYPE_MOVETO_CURSOR:
			wlr_log(WLR_ERROR,
				"Action MoveToCursor is deprecated. To ensure your config works in future wrdp-compositor "
