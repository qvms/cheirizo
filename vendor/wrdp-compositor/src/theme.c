...
/*
 * Theme engine for wrdp-compositor
 *
...
static void
os9_set_cairo_rgb(cairo_t *cairo, uint32_t rgb)
{
...
static void
os9_button_rect(cairo_t *cairo, int x, int y, int w, int h, uint32_t rgb)
{
	if (w <= 0 || h <= 0) {
		return;
	}
	os9_set_cairo_rgb(cairo, rgb);
...
struct os9_platinum_button_data {
	enum ssd_part_type type;
	bool active;
};

static void
os9_platinum_button_draw_classic(cairo_t *cairo,
		enum ssd_part_type type, bool active)
{
	/* Classic Platinum controls are 13x13 px in a roughly 20 px titlebar.
	 * Draw them directly at that authoring size instead of resampling the
	 * newer 24px procedural source. */
	cairo_set_operator(cairo, CAIRO_OPERATOR_CLEAR);
	cairo_paint(cairo);
	cairo_set_operator(cairo, CAIRO_OPERATOR_OVER);
	cairo_set_antialias(cairo, CAIRO_ANTIALIAS_NONE);

	uint32_t outline = active ? 0x111111 : 0x575757;
	uint32_t face = active ? 0xd8d8d8 : 0xcfcfcf;
	uint32_t hi = active ? 0xffffff : 0xe6e6e6;
	uint32_t mid_hi = active ? 0xeeeeee : 0xd8d8d8;
	uint32_t shade = active ? 0x909090 : 0xa0a0a0;
	uint32_t shadow = active ? 0x303030 : 0x707070;
	uint32_t ink = active ? 0x000000 : 0x575757;

	os9_button_rect(cairo, 0, 0, 13, 13, outline);
	os9_button_rect(cairo, 1, 1, 11, 11, face);
	os9_button_rect(cairo, 1, 1, 11, 1, hi);
	os9_button_rect(cairo, 1, 1, 1, 11, hi);
	os9_button_rect(cairo, 2, 2, 9, 1, mid_hi);
	os9_button_rect(cairo, 2, 2, 1, 9, mid_hi);
	for (int y = 3; y < 10; ++y) {
		int v = active ? 207 + (y - 3) * 3 : 202;
		uint32_t rgb = ((uint32_t)v << 16) | ((uint32_t)v << 8) | (uint32_t)v;
		os9_button_rect(cairo, 3, y, 7, 1, rgb);
	}
	os9_button_rect(cairo, 1, 11, 11, 1, shade);
	os9_button_rect(cairo, 11, 1, 1, 11, shade);
	os9_button_rect(cairo, 2, 10, 9, 1, shadow);
	os9_button_rect(cairo, 10, 2, 1, 9, shadow);

	if (type == LAB_SSD_BUTTON_ICONIFY) {
		/* Collapse box horizontal stroke. */
		os9_button_rect(cairo, 3, 8, 7, 1, ink);
		os9_button_rect(cairo, 3, 9, 7, 1, active ? 0x242424 : ink);
	} else if (type == LAB_SSD_BUTTON_MAXIMIZE) {
		/* Zoom box corner glyph. */
		os9_button_rect(cairo, 7, 3, 1, 6, ink);
		os9_button_rect(cairo, 8, 3, 1, 6, active ? 0x606060 : ink);
		os9_button_rect(cairo, 3, 7, 5, 1, ink);
		os9_button_rect(cairo, 3, 8, 6, 1, active ? 0x404040 : ink);
	} else if (type == LAB_SSD_BUTTON_CLOSE) {
		/* Close box: small recessed square, not an X. */
		os9_button_rect(cairo, 4, 4, 5, 1, ink);
		os9_button_rect(cairo, 4, 4, 1, 5, ink);
		os9_button_rect(cairo, 4, 8, 5, 1, active ? 0x606060 : ink);
		os9_button_rect(cairo, 8, 4, 1, 5, active ? 0x606060 : ink);
	} else if (type == LAB_SSD_BUTTON_SHADE) {
		/* Windowshade box: a small recessed rectangle with a single
		 * horizontal bar near the top, evoking a window rolled up into its
		 * titlebar (distinct from the collapse/iconify glyph). */
		os9_button_rect(cairo, 4, 4, 5, 1, ink);
		os9_button_rect(cairo, 4, 5, 5, 1, active ? 0x242424 : ink);
	}
}

static struct lab_data_buffer *
os9_platinum_button_render(void *data, int width, int height, double scale)
{
	struct os9_platinum_button_data *button = data;
	...
	cairo_paint(cairo);
	cairo_set_operator(cairo, CAIRO_OPERATOR_OVER);
	int sx = button->active ? 0 : 14;
	int sy = 0;
	if (button->type == LAB_SSD_BUTTON_ICONIFY
			|| button->type == LAB_SSD_BUTTON_SHADE) {
		/* In classic Mac OS the windowshade (shade) control and the collapse
		 * box share the same box-with-divider glyph. */
		sx = button->active ? 28 : 42;
	} else if (button->type == LAB_SSD_BUTTON_MAXIMIZE) {
		sx = button->active ? 56 : 70;
	}
	bool atlas_ok = os9_draw_atlas(cairo, sx, sy, 13, 13, 0, 0, width, height);
	if (!atlas_ok) {
		cairo_scale(cairo, (double)width / 13.0, (double)height / 13.0);
		os9_platinum_button_draw_classic(cairo, button->type, button->active);
	} else if (button->type == LAB_SSD_BUTTON_SHADE) {
		/*
		 * Windowshade divider: the atlas bakes it as a 1px source row that
		 * scales up to ~2px. Draw it procedurally instead as a crisp single
		 * output-pixel black line (half the baked thickness), spanning the
		 * inner box (atlas cols 2..10 of 13, row 8 of 13).
		 */
		int dy = (int)(8.0 * height / 13.0 + 0.5);
		int dx0 = (int)(2.0 * width / 13.0 + 0.5);
		int dx1 = (int)(11.0 * width / 13.0 + 0.5);
		cairo_antialias_t prev = cairo_get_antialias(cairo);
		cairo_set_antialias(cairo, CAIRO_ANTIALIAS_NONE);
		cairo_set_source_rgb(cairo, 0, 0, 0);
		cairo_rectangle(cairo, dx0, dy, dx1 - dx0, 1);
		cairo_fill(cairo);
		cairo_set_antialias(cairo, prev);
	}
...
os9_platinum_button_img(enum ssd_part_type type, bool active)
{
	struct os9_platinum_button_data *data = znew(*data);
	data->type = type;
	data->active = active;
	return lab_img_create_procedural(os9_platinum_button_render, data, free);
}
...
load_button(struct theme *theme, struct button *b, int active)
{
	...
	if (os9_platinum_theme_enabled()
			&& !(b->state_set & LAB_BS_HOVERD)
			&& (b->type == LAB_SSD_BUTTON_CLOSE
				|| b->type == LAB_SSD_BUTTON_ICONIFY
				|| b->type == LAB_SSD_BUTTON_MAXIMIZE
				|| b->type == LAB_SSD_BUTTON_SHADE
				|| b->type == LAB_SSD_BUTTON_WINDOW_MENU)) {
		*img = os9_platinum_button_img(b->type, active == THEME_ACTIVE);
		if (*img) {
			goto loaded;
		}
	}
...
parse_hexstr(const char *hex, float *rgba)
{
	...
	if (len >= 9 && hex[7] == ' ') {
		...
		wlr_log(WLR_ERROR,
			...
			"#rrggbbaa to ensure your config works on newer wrdp-compositor releases", hex);
	} else if (len == 9) {
 ...
 * packaged with Clearlooks, this is not a problem, but for wrdp-compositor I thought
 * this was a bit hard-line. People might want to try wrdp-compositor without having
...
static int
os9_scaled_metric(int classic_value, float client_scale)
{
	return MAX(1, (int)((float)classic_value * client_scale + 0.5f));
}

static void
os9_platinum_apply_metrics(struct theme *theme)
{
	if (!os9_platinum_theme_enabled()) {
		return;
	}

	/* XFWM/PlatiNomNom frame metrics, scaled to client density. */
	float client_scale = os9_platinum_client_scale();
	theme->border_width = os9_scaled_metric(6, client_scale);
	theme->window_button_width = os9_scaled_metric(13, client_scale);
	theme->window_button_height = os9_scaled_metric(13, client_scale);
	theme->window_button_spacing = os9_scaled_metric(3, client_scale);
	theme->window_titlebar_padding_width = 0;
	theme->window_titlebar_padding_height = 0;
}
...
{
	os9_platinum_apply_metrics(theme);
	theme->titlebar_height = get_titlebar_height(theme);
	if (os9_platinum_theme_enabled()) {
		theme->titlebar_height = os9_scaled_metric(22, os9_platinum_client_scale());
	}
...
theme_init(struct theme *theme, struct server *server, const char *theme_name)
{
	...
	if (theme_name) {
		/*
		 * Read
		 *   - <data-dir>/share/themes/$theme_name/wrdp-compositor/themerc
	...
	/* Read <config-dir>/wrdp-compositor/themerc-override */
