...
#endif
	lab_img_render_func_t render;
	lab_img_data_destroy_func_t destroy;
	void *render_data;
};
...
struct lab_img *
lab_img_create_from_buffer(struct lab_data_buffer *buffer)
{
	if (!buffer) {
		return NULL;
	}
	struct lab_img_data *img_data = znew(*img_data);
	img_data->type = LAB_IMG_XPM;
	img_data->buffer = buffer;
	return create_img(img_data);
}

struct lab_img *
lab_img_create_procedural(lab_img_render_func_t render, void *data,
	lab_img_data_destroy_func_t destroy)
{
	if (!render) {
		return NULL;
	...
	struct lab_img_data *img_data = znew(*img_data);
	img_data->type = LAB_IMG_PROCEDURAL;
	img_data->render = render;
	img_data->render_data = data;
	img_data->destroy = destroy;
	return create_img(img_data);
}
...
#endif
	case LAB_IMG_PROCEDURAL:
		buffer = img->data->render(img->data->render_data,
			width, height, scale);
		break;
	default:
		break;
...
#endif
		if (img->data->destroy) {
			img->data->destroy(img->data->render_data);
		}
		free(img->data);
	}
