...
static void
build_theme_path_wrdp_compositor(struct ctx *ctx, char *prefix, const char *path)
{
	assert(prefix);
	snprintf(ctx->buf, ctx->len, "%s/%s/%s/wrdp-compositor/%s", prefix, path,
...
{
	char *debug = getenv("WRDP_COMPOSITOR_DEBUG_DIR_CONFIG_AND_THEME");
		...
		const char *filename)
{
	...
	wl_list_init(paths);
	struct ctx ctx = {
		.build_path_fn = build_theme_path_wrdp_compositor,
		.filename = filename,
		.buf = buf,
		.len = sizeof(buf),
		.dirs = theme_dirs,
