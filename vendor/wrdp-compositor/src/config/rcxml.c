...
/*
 * Openbox/wrdp-compositor comparison
 *
 ...
 * identifier.
 *
 * wrdp-compositor        openbox     description
 * -----        -------     -----------
...
entry(xmlNode *node, char *nodename, char *content, struct parser_state *state)
{
	...
	string_truncate_at_pattern(nodename, ".wrdp_compositor_config");

	if (getenv("WRDP_COMPOSITOR_DEBUG_CONFIG_NODENAMES")) {
		printf("%s: %s\n", nodename, content);
	}
	...
	if (!strcmp(nodename, "decoration.core")) {
		...
		if (!rc.xwayland_persistence) {
			wlr_log(WLR_ERROR, "to avoid the risk of a fatal crash, "
				...
				"recommended when wrdp-compositor is compiled against "
...
post_processing(void)
{
	...
	if (!rc.fallback_app_icon_name) {
		rc.fallback_app_icon_name = xstrdup("wrdp-compositor");
	}
