...
/*
 * output.c: wrdp-compositor output and rendering
 *
...
new_output_notify(struct wl_listener *listener, void *data)
{
	...
	if (wlr_output_is_wl(wlr_output)) {
		char title[64];
		snprintf(title, sizeof(title), "%s - %s", "wrdp-compositor", wlr_output->name);
		wlr_wl_output_set_title(wlr_output, title);
		wlr_wl_output_set_app_id(wlr_output, "wrdp-compositor");
	}
...
handle_output_power_manager_set_mode(struct wl_listener *listener, void *data)
{
	...
	case ZWLR_OUTPUT_POWER_V1_MODE_ON:
		...
		if (!event->output->current_mode) {
			 ...
			 *       wrdp-compositor starts tracking wlroots 0.19 to keep older
