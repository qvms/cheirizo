...
#ifndef WRDP_COMPOSITOR_IMG_H
#define WRDP_COMPOSITOR_IMG_H
...
/**
 * lab_img_create_from_buffer() - create an image from an already rendered Cairo buffer.
 * The image takes ownership of @buffer and releases it via wlr_buffer_drop().
 */
struct lab_img *lab_img_create_from_buffer(struct lab_data_buffer *buffer);

typedef struct lab_data_buffer *(*lab_img_render_func_t)(void *data,
	int width, int height, double scale);
typedef void (*lab_img_data_destroy_func_t)(void *data);

/**
 * lab_img_create_procedural() - create an image rendered by code at target scale.
 * @render: callback that returns a buffer for the requested logical size/scale
 * @data: callback-owned data shared by copied images
 * @destroy: optional data destructor called with the final image reference
 */
struct lab_img *lab_img_create_procedural(lab_img_render_func_t render,
	void *data, lab_img_data_destroy_func_t destroy);
