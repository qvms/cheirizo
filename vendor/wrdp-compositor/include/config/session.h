...
#ifndef WRDP_COMPOSITOR_SESSION_H
#define WRDP_COMPOSITOR_SESSION_H
 ...
 * pairs in `${XDG_CONFIG_DIRS:-/etc/xdg}/wrdp-compositor/environment` with user override
 ...
 * Note: Same as `sh ~/.config/wrdp-compositor/autostart` (or equivalent XDG config dir)
 */
void session_autostart_init(struct server *server);

/**
 * session_shutdown - run session shutdown file as shell script
 * Note: Same as `sh ~/.config/wrdp-compositor/shutdown` (or equivalent XDG config dir)
 */
void session_shutdown(struct server *server);

#endif /* WRDP_COMPOSITOR_SESSION_H */
