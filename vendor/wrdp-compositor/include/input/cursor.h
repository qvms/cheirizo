...
#ifndef WRDP_COMPOSITOR_CURSOR_H
#define WRDP_COMPOSITOR_CURSOR_H
...
/* Cursors used internally by wrdp-compositor */
enum lab_cursors {
	LAB_CURSOR_CLIENT = 0,
	LAB_CURSOR_DEFAULT,
	LAB_CURSOR_GRAB,
...
/**
 * cursor_update_image - re-set the wrdp-compositor cursor image
 * @seat - seat
 *
 * This can be used to update the cursor image on output scale changes.
 * If the current cursor image was not set by wrdp-compositor but some client
 * this is a no-op.
 */
...
#endif /* WRDP_COMPOSITOR_CURSOR_H */
