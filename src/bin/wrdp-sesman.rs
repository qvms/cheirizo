//! `wrdp-sesman` CLI entrypoint for managing per-user wrdp sessions.
...
use wrdp::sesman::{EnsureOptions, SesmanConfig, SessionManager, SessionSize};

#[derive(Debug, Parser)]
#[command(name = "wrdp-sesman")]
#[command(version, about = "wrdp session manager", long_about = None)]
struct Args {
    /// Optional sesman INI config path.
    #[arg(short, long)]
...
enum Command {
    ...
    /// Record reconnect metadata while reusing an existing healthy session.
    Reconnect {
        /// Requested client desktop width.
        #[arg(long)]
        width: Option<u32>,
    ...
    /// Ensure the managed session exists, then monitor it in the foreground.
    Run {
    ...
    /// Print the effective default INI config.
    GenerateConfig,
}
...
fn main() -> Result<()> {
    ...
    if matches!(args.command, Command::GenerateConfig) {
        let ini = SesmanConfig::generate_default_ini();
        print!("{ini}");
        return Ok(());
    }
...
/// Poll sesman state in the foreground and fail fast if health degrades.
fn monitor_foreground(manager: &SessionManager, poll_seconds: u64) -> Result<()> {
    ...
    loop {
        ...
        let status = manager.status()?;
        if status.health != wrdp::sesman::SessionHealth::Healthy {
            anyhow::bail!(
...
fn init_logging(verbose: u8) {
    ...
    if tracing::subscriber::set_global_default(subscriber).is_ok() {
        info!("wrdp-sesman logging initialized");
    }
}
