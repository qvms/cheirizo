//! Shared data types for the runtime XDG portal backend.
//!
//! Defines the common source/stream/input/clipboard/cursor types exchanged
//! between portal D-Bus interfaces and backend service layers.
...
pub struct StreamInfo {
    ...
    ///
    /// Format: `"output:<name>"` (for example `"output:eDP-1"`). Used to let
    /// clients restore saved source selections.
...
///
/// Used for multi-monitor absolute pointer positioning, translating
/// normalized stream coordinates into compositor-global coordinates.
#[derive(Debug, Clone)]
