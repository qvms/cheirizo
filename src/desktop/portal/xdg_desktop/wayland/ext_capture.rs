...
//!
//! Manages ext-capture session lifecycle, constraint negotiation, SHM buffer
//! allocation, and frame handoff to PipeWire or screenshot reply paths.
...
use crate::desktop::portal::xdg_desktop::{pipewire::PipeWireManager, wayland::ScreenshotData};
...
/// Buffer constraints collected during session negotiation.
///
/// Built from `buffer_size` and `shm_format` events until the session
/// emits `done`.
...
/// State for one active ext-capture session.
