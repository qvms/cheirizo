//! Wayland data-control clipboard state and protocol management.
//!
//! Maintains clipboard integration across the Wayland event loop and portal
//! D-Bus runtime paths, handling selection offers, source publication, and
...
///
/// Owns protocol object lifecycle and routes clipboard events into shared
/// state visible to portal-facing code.
...
impl DataControlState {
    ...
    fn set_pending_offer(&mut self, offer: DataControlOffer) {
        // Destroy any pending offer that wasn't used
    ...
    pub fn on_selection(&mut self) {
        // Destroy the current offer before replacing it
    ...
    pub fn on_source_send(&self, mime_type: &str, fd: OwnedFd) {
        ...
        if let Some(data) = data {
            ...
            tracing::warn!("Source send event for unknown MIME type");
    ...
    ///
    /// Creates a new data-control source, advertises MIME types, and sets it
    /// as the active compositor selection.
    pub fn set_selection(
        ...
        // Destroy the existing source before replacing it
        if let Some(source) = self.current_source.take() {
            source.destroy();
        }
    ...
    pub fn receive_from_offer(&self, mime_type: &str, fd: OwnedFd) {
        match &self.current_offer {
            Some(offer) => {
                ...
                tracing::debug!("Requested clipboard data from compositor offer");
            }
            None => {
                tracing::warn!("ReceiveFromOffer but no current offer");
