//! Session state types and lifecycle transitions for portal runtime sessions.
...
///
/// Carries source-selection identity across session restarts when
...
/// A portal session.
///
/// Tracks lifecycle state and resources for capture, input, and clipboard
/// runtime flows.
#[derive(Debug)]
pub struct Session {
    ...
    /// Restore data from an earlier session: output names that were selected.
...
impl Session {
    ...
    pub fn select_sources(&mut self, sources: Vec<SourceInfo>) -> Result<()> {
        ...
        tracing::debug!(source_count = sources.len(), "Sources selected");
    ...
    pub fn request_clipboard(&mut self) -> Result<()> {
        ...
        tracing::debug!("Clipboard enabled");
        Ok(())
    }
    ...
    pub fn start(&mut self, streams: Vec<StreamInfo>) -> Result<()> {
        ...
        tracing::info!(stream_count = self.streams.len(), "Session started");
        Ok(())
    }
    ...
    pub fn connect_to_eis(&mut self, context_id: u32) -> Result<()> {
        ...
        tracing::debug!(context_id = context_id, "EIS connected");
        Ok(())
    }
...
mod tests {
    ...
    fn test_cannot_start_without_selection() {
        ...
        session2
            .select_sources(vec![
                crate::desktop::portal::xdg_desktop::types::SourceInfo {
                    id: 1,
                    name: "Test".to_string(),
