...
//!
//! Each stream corresponds to one captured output. Streams are created as
//! `Video/Source` nodes, with PipeWire-managed buffer allocation, and frames
//! are queued by copying capture data into dequeued buffers.
...
///
/// Wraps a `pw_stream` that advertises as a `Video/Source` node for portal
/// capture consumers.
...
impl PipeWireVideoStream {
    ...
    ///
    /// Creates a format object describing raw BGRx frames with fixed dimensions
    /// and frame rate.
    ...
    ///
    /// Dequeues a PipeWire buffer, copies frame bytes, updates chunk metadata,
    /// then relies on RAII to return the buffer to PipeWire.
    pub fn queue_frame(
        &mut self,
        data: &[u8],
        _width: u32,
