...
pub struct WlrCaptureBackend {
    ...
    /// Whether to bypass PipeWire stream creation and deliver frames directly.
    direct_frame_channel: bool,
}
...
impl CaptureBackend for WlrCaptureBackend {
    ...
    fn create_capture_session(
        ...
        for source in sources {
            ...
            let node_id = if self.direct_frame_channel {
                // In direct-frame mode the node ID is only an in-process stream key
                // used to associate frames with an RDP monitor. No PipeWire daemon
                // or real PipeWire stream is required.
                source.id.max(1)
            } else {
                // Create a real PipeWire stream via the manager
                let config = crate::desktop::portal::xdg_desktop::pipewire::StreamConfig {
                    source_id: source.id,
                    width: source.width,
            ...
            if self.direct_frame_channel {
                tracing::info!(
                    node_id = node_id,
                    source_id = source.id,
                    source_name = %source.name,
                    protocol = "wlr-screencopy",
                    "Created direct-frame capture stream"
                );
            } else {
                tracing::info!(
                    node_id = node_id,
            ...
            // pixel data through the configured transport (direct frame channel or PipeWire).
