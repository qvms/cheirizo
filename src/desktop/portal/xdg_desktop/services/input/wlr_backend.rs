...
/// sharing the main `WaylandConnection` from [`crate::desktop::portal::xdg_desktop::wayland`]. This is
/// intentional:
///
...
/// - This uses a dedicated Wayland connection per backend that needs to send
///   requests from D-Bus handler context.
///
...
impl WlrInputBackend {
    ...
    pub fn new(config: &WlrConfig) -> Result<Self> {
        ...
        let connection = if let Some(ref path) = config.wayland_socket_path {
            use std::os::unix::net::UnixStream;

            tracing::info!("Connecting wlr virtual input backend to Wayland compositor");
        ...
        } else if let Some(ref wayland_display) = config.wayland_display {
...
impl InputBackend for WlrInputBackend {
    ...
    fn create_context(
        ...
        if devices.pointer {
            if let Some(pointer) = self.create_pointer() {
                tracing::debug!("Created virtual pointer");
    ...
    fn destroy_context(&mut self, session_id: &str) -> Result<()> {
        if let Some(ctx) = self.sessions.remove(session_id) {
            ...
            self.flush()?;
            tracing::info!("wlr virtual input context destroyed");
        }
        Ok(())
    }
    ...
    fn keysym_to_keycode(&self, keysym: u32) -> Option<u32> {
        ...
        tracing::warn!("No keycode found for received keysym");
        None
    }
