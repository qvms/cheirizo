...
//!
//! ```text
//! Client (EIS) --[EIS socket]--> EisBridgeBackend --[wlr virtual input]--> Compositor
//! ```
//!
...
impl InputBackend for EisBridgeBackend {
    ...
    fn create_context(
        ...
        tracing::info!("EIS bridge context created (EIS server + wlr virtual devices)");
    ...
    fn destroy_context(&mut self, session_id: &str) -> Result<()> {
        if self.sessions.remove(session_id).is_some() {
            tracing::info!("EIS bridge context destroyed");
        }
