...
pub trait InputBackend: Send + Sync {
    ...
    fn set_stream_mappings(
        &mut self,
        _mappings: Vec<crate::desktop::portal::xdg_desktop::types::StreamOutputMapping>,
    ) {
...
pub struct WlrConfig {
    ...
    /// Explicit Wayland Unix socket path to connect to.
    ///
...
pub fn create_input_backend(
    ...
    wayland_protocols: &crate::desktop::portal::xdg_desktop::wayland::AvailableProtocols,
    ...
    match protocol {
        InputProtocol::Eis => {
            ...
            if !wayland_protocols.wlr_virtual_pointer && !wayland_protocols.zwp_virtual_keyboard {
                return Err(
                    crate::desktop::portal::xdg_desktop::error::PortalError::Config(
...
mod tests {
    ...
    #[test]
    fn test_managed_input_kind_display() {
