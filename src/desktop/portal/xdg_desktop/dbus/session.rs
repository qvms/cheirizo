...
//!
//! Implements the runtime session-lifecycle boundary for portal sessions,
//! including close handling and cleanup signaling.
...
///
/// Registered at the session handle path while a portal session is active.
/// Provides `Close()` and emits `Closed` after cleanup.
pub struct SessionInterface {
    ...
    /// Capture backend for destroying capture streams on close.
