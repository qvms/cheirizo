...
After authentication and desktop-size negotiation, IronRDP calls the connection binder with the accepted credentials and negotiated `DesktopSize`. The binder normalizes the username, asks session management to start or reuse that user's managed desktop, applies the negotiated size to the compositor output before capture, records the client attachment, and returns handlers bound to that session.
